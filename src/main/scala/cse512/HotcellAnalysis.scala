  ...
  def runHotcellAnalysis(spark: SparkSession, pointPath: String): DataFrame =
    {
      ...
      val givDf = spark.sql("select x,y,z from pickupinfo where x>=" + minX + " and x<= " + maxX + " and y>= " + minY + " and y<= " + maxY + " and z>= " + minZ + " and z<= " + maxZ + " order by z,y,x").persist();
      givDf.createOrReplaceTempView("Df0")

      val givDf1 = spark.sql("select x,y,z,count(*) as pointval from Df0 group by z,y,x order by z,y,x").persist();
      givDf1.createOrReplaceTempView("Df1")

      spark.udf.register("squared", (inputX: Int) => ((HotcellUtils.squared(inputX))))

      val sumPts = spark.sql("select count(*) as countval, sum(pointval) as sumval,sum(squared(pointval)) as squaredsum from Df1");
      sumPts.createOrReplaceTempView("sumPts")

      val actSum0 = sumPts.first().getLong(1);
      val actSum1 = sumPts.first().getDouble(2);
      val actSum2 = sumPts.first().getLong(0);

      val mean =(actSum0.toDouble / numCells.toDouble).toDouble;
      val SD = math.sqrt(((actSum1.toDouble / numCells.toDouble) - (mean.toDouble * mean.toDouble))).toDouble
      ...
      val Nbrs1 = spark.sql("select GScore(x,y,z,"+mean+","+SD+",ncount,sumtotal,"+numCells+") as gtstat,x, y, z from Df2 order by gtstat desc");
